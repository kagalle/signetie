...
// TODO: the authenticate part of this might as well move into AuthenticateResult, and AuthenticateResult be changed to authenciateResult.
// that leaves this class to call out to do the authenticate if needed, followed by the code/token exchange.

// CodeTokenComplete defines a user-supplied function to be called
// at completion of the process.
type CodeTokenComplete func(auth *AuthenticateResult, port int)
...
func RequestAuthentication(parentWindow *gtk.Window, scope string, clientID string, port int, authCompleteCallback CodeTokenComplete) (err error) {

	auth := new(AuthenticateResult)
